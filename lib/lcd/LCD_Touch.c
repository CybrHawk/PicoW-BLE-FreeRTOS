...
void drawButton(Button button) {
	...
	GUI_DrawRectangle(button.x,button.y,button.width,button.height, MAGENTA, DRAW_FULL, DOT_PIXEL_1X1);
...
bool isButtonPressed(Button button, LCD_SCAN_DIR lcd_scan_dir) {
	...
	if (sTP_DEV.chStatus & TP_PRESS_DOWN) { // Press the button
		uint16_t x = sTP_Draw.Xpoint;
		uint16_t y = sTP_Draw.Ypoint;
		//sprintf(coord_str, "X:%d, Y:%d", x, y); //Debugging coordinates
		//GUI_DisString_EN(10, 290, coord_str, &Font24, FONT_BACKGROUND, BLUE);

		if (x >= button.x && x <= (button.x + button.width) && y >= button.y && y <= (button.y + button.height)) {
			LCD_Clear(LCD_BACKGROUND); //Debugging purposes
			return true;
		}
	}
	return false;
